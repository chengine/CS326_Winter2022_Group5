...
class LocobotFollowTrajectory(object):
    ...
    def __init__(self):
        ...
        self.r_xy = lambda t : (0.5 * np.cos(t * 2*np.pi/10), 0.5 * np.sin(t * 2*np.pi/10))
        self.t_init = None
    ...
    def mobile_base_callback(self, data):
        ...
        self.t_init = self.t_init or rospy.Time.now()
        self.target_pose.position.x, self.target_pose.position.y = self.r_xy((rospy.Time.now()-self.t_init).to_sec())
        print((rospy.Time.now()-self.t_init).to_sec())
...
def main():
    ...
    cls_obj = LocobotFollowTrajectory() #instantiate object of the class (to call and use the functions)
    cls_obj.run()
